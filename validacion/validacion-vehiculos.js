...
const validarActualizarVehiculo = [
    body('patente')
        .optional().matches(/(^[a-zA-Z]{3}[-]{0,1}[0-9]{3,4}$)|(^[a-zA-Z]{2}[-]{0,1}[0-9]{3}[a-zA-Z]{1}$)/).withMessage('Formato de patente inválido'),
    body('marca')
        .optional().matches(/^[A-Za-zÀ-ÿ\s]+$/).withMessage('La marca debe contener solo letras y espacios').notEmpty().withMessage('La marca no puede estar vacía'),
    body('modelo')
        .optional().matches(/^[A-Za-zÀ-ÿ0-9\s\+\.\-]+$/).withMessage('El modelo debe contener solo letras y espacios numeros y signo (+ . -)').notEmpty().withMessage('El modelo no puede estar vacío'),
    body('color')
        .optional(),
    body('usuario_id')
        .optional().isInt().withMessage('El ID de usuario debe ser un número entero'),
    body('permitir_valet')
        .optional().isBoolean().withMessage('Debe ser true o false'),
    // ... middleware para revisar los resultados
    verificarErrores 
];
